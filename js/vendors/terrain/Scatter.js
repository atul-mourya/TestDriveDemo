 ...
 */
const ScatterMeshes = function ( geometry, options ) {
	...
	for ( var i = 0; i < geometry.attributes.position.array.length; i += 9 ) {
		...
		if ( place ) {
			...
			// Don't place a mesh if the maximum number of meshes has been reached
			if ( meshCount >= options.maxMeshes ) {
			...
			mesh.updateMatrix();
			instanceMesh.setMatrixAt( meshCount + 1, mesh.matrix );

			meshCount ++; // Increment the mesh counter
 ...
 *   function.
 */
const ScatterHelper = function ( method, options, skip, threshold ) {
...
export { ScatterMeshes, ScatterHelper };
